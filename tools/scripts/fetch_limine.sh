...
git -C "$LIMINE_DIR" rev-parse HEAD \
    | tee build/meta/limine-revision.txt

printf 'branch=%s\nurl=%s\n' \
    "$LIMINE_BRANCH" \
    "$LIMINE_URL" \
    | tee -a build/meta/limine-revision.txt
...
test -x "$LIMINE_DIR/limine" || \
test -f "$LIMINE_DIR/limine"
