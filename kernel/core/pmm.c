...
static void mark_range_free(
    ...
    if (length == 0
        || !checked_add_u64(base, length, &end)) {
        return;
    }

    uint64_t start = align_up(
        base,
        PMM_PAGE_SIZE
    );

    uint64_t stop = align_down(
        end,
        PMM_PAGE_SIZE
    );
    ...
    if (stop > pmm->max_phys) {
        ...
        pmm->ignored_frames +=
            (align_down(end, PMM_PAGE_SIZE) - stop)
            / PMM_PAGE_SIZE;
    }
...
static void mark_range_used(
    ...
    uint64_t start = align_down(
        base,
        PMM_PAGE_SIZE
    );

    uint64_t stop = align_up(
        end,
        PMM_PAGE_SIZE
    );
...
bool pmm_init_from_map(
    ...
    if (max_phys_bytes == 0
        || (max_phys_bytes
        & (PMM_PAGE_SIZE - 1ULL)) != 0) {
        return false;
    }

    uint64_t frame_count =
        max_phys_bytes / PMM_PAGE_SIZE;

    uint64_t required_bitmap_bytes =
        (frame_count + 7ULL) / 8ULL;

    if (bitmap_storage_bytes
...
uint64_t pmm_alloc_frame(
    ...
    for (uint64_t frame = pmm->next_hint;
         frame < pmm->frame_count;
         frame++) {
