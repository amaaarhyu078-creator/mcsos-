...
static uint64_t *table_from_phys(
...
static int get_or_alloc_next_table(
    struct vmm_space *space,
    uint64_t *table,
    unsigned index,
    uint64_t **out)
{
    ...
    uint64_t new_paddr =
        space->alloc_frame(space->ctx);

    if (new_paddr == VMM_INVALID_PHYS ||
        !vmm_is_aligned_4k(new_paddr))
    {
        return VMM_ERR_NOMEM;
    }
